...
export function ContactClient({ departments }: { departments: IDepartment[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitError(null);
    try {
      const dept = departments.find((d) => d.id === formState.department);
      await submitContact({
        name: formState.name.trim(),
        phone: formState.phone.trim(),
        departmentId: dept?.dbId,
            ...
            <div className="lg:col-span-6 bg-white rounded-3xl p-5 sm:p-8 lg:p-10 border border-slate-200/80 shadow-sm">
              ...
              {isSubmitted ? (
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {submitError && (
                    <p role="alert" className="text-xs font-semibold text-red-600 bg-red-50 border border-red-100 rounded-xl px-4 py-2.5">
                      {submitError}
                    </p>
                  )}
