...
export function Navbar({ onOpenAppointment }: NavbarProps) {
  ...
  return (
    <>
                ...
                >
                  ...
                  {isActive && (
                    ...
                    <m.span
                      ...
                      className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-5 h-[2px] bg-accent rounded-full shadow-[0_1px_3px_rgba(220,38,38,0.3)]"
                    />
                  )}
                </Link>
              );
          ...
          <div className="flex items-center gap-1.5 sm:gap-3 shrink-0">
                    ...
                    {LANGUAGES.map((lang) => {
                      ...
                      return (
                        <button
                          ...
                          role="menuitem"
                          onClick={() =>
                            handleLanguageChange(lang.code as "uz" | "kr")
                          }
            ...
            <div className="hidden sm:block">
              <MagneticButton
                ...
                strength={10}
                glow
                className="btn-ekg inline-flex items-center gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-full text-xs sm:text-sm font-bold text-white bg-accent hover:bg-accent-hover transition-colors duration-200 shadow-md shadow-red-500/20 active:scale-95 cursor-pointer shrink-0"
              >
            ...
            <button
              ...
              className="sm:hidden w-8 h-8 rounded-full bg-accent hover:bg-accent-hover text-white flex items-center justify-center shadow-md shadow-red-500/25 active:scale-90 transition-all cursor-pointer shrink-0"
            >
      ...
      {/* Drawer va backdrop header'dan TASHQARIDA: header `transform` bilan animatsiya
          qilinadi, transform'li ota ichidagi `position: fixed` viewport'ga emas,
          o'sha otaga (60px) nisbatan joylashib qoladi. */}
      {/* ---------------- Mobil drawer ---------------- */}
      <AnimatePresence>
        {isMobileMenuOpen && (
          <>
            ...
            >
              ...
              <div className="pt-4 border-t border-white/10 space-y-3 shrink-0">
                ...
                <button
                  ...
                  className="w-full py-3.5 rounded-2xl bg-accent hover:bg-accent-hover active:scale-[0.98] text-white font-bold text-sm shadow-lg shadow-red-600/30 flex items-center justify-center gap-2 transition-all cursor-pointer"
                >
                  <Calendar className="w-4 h-4" />
                  <span>{t("appointment")}</span>
                </button>
